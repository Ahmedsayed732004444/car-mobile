...
class VendorCitiesScreen extends StatefulWidget {
  const VendorCitiesScreen({super.key});

  @override
  State<VendorCitiesScreen> createState() =>
      _VendorCitiesScreenState();
}

class _VendorCitiesScreenState extends State<VendorCitiesScreen> {
    ...
    return Scaffold(
      ...
      body: _provider.isLoadingRefresh
        ...
        child: RefreshIndicator(
          onRefresh: (){
            return _provider.getVendorCities(context);
          },
          child: ListView(
            physics:  AlwaysScrollableScrollPhysics(),
            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 50),
            children: [
              Text('إختر المدن التي تتوفر فيها خدماتك'),
              const SizedBox(
                height: 30,
              ),
              Row(
                children: [
                  ...
                  Expanded(
                    child: CitiesDropdownSearch(
                      filled: true,
                      fillColor: AppColor.secondaryColor,
                      selectedCities: _provider.selectedCitiesList,
                      onChanged: (List<CityModel>? selectedItems) {
                        _provider.changeCities(selectedItems);
                      },
                    ),
                  )
                ],
  ...
  Future<void> _execute(BuildContext context, SpecialtyVendorProvider provider)async {
    ...
    if (myValid) {
      ...
      await provider.updateVendorCities();
    }
  }
}
