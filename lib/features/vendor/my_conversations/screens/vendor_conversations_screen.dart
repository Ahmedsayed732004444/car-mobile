...
class VendorConversationScreen extends StatefulWidget {
  const VendorConversationScreen({super.key});

  @override
  _VendorConversationScreenState createState() => _VendorConversationScreenState();
}

class _VendorConversationScreenState extends State<VendorConversationScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      await provider.getMyConversations(path: ApiEndpoints.vendorConversations);
    });
    ...
    _scrollController.addListener(() {
      ...
      if (_scrollController.position.pixels ==
          ...
          !p.isLoading &&
          p.hasMore) {
        p.getMyConversations( path: ApiEndpoints.vendorConversations,loadMore: true);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("محادثاتي", style: txtMedium05,),
      ...
      body: Consumer<ConversationProvider>(
        builder: (context, provider, child) {
          ...
          return RefreshIndicator(
            onRefresh: () => provider.refreshGetMyConversations(ApiEndpoints.vendorConversations),
